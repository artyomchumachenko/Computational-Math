...
public class Task4 {
    ...
    private static void minimumDiscrepancyOrSpeedDescentMethod(String methodName, double[][] matrix, int norm) {
        ...
        do {
            RealVector Axk = aMatrix.operate(xValuesNew);
            RealVector residual = Axk.subtract(fValues);
            double tau = 0;
            if (methodName.equals("МН")) {
                tau = getTauForDisperancy(residual, aMatrix);
            } else {
                tau = getTauForSpeedDescent(residual, aMatrix);
            }
        ...
        switch (numOfNorm) {
            ...
            case (0) -> {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    // Нахождение максимума в первом проходе массива
                    //
                }
